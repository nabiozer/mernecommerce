...
import { listUsersHandler ,deleteUserHandler } from '../store/user-actions';

const UserListPage = () => {
    const history = useHistory();
    const dispatch = useDispatch();
    const user = useSelector(state => state.user)
    const {users, error, loading ,succes} = user;
    ...
    useEffect(() => {
        ...
        dispatch(listUsersHandler());

        } else {
            history.push('/login')
        }
    }, [dispatch,history,succes]);
  return (
    <>
        <h1>Users</h1>
